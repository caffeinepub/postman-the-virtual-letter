...
export class Backend implements backendInterface {
    ...
    async searchProfilesByName(arg0: string): Promise<Array<Profile>> {
        if (this.processError) {
            try {
    ...
    async sendLetter(arg0: Principal, arg1: string, arg2: StampType): Promise<bigint> {
        if (this.processError) {
            try {
                const result = await this.actor.sendLetter(arg0, arg1, to_candid_StampType_n6(this._uploadFile, this._downloadFile, arg2));
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function to_candid_variant_n7(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: StampType): {
    indian: null;
} | {
    pakistani: null;
} {
    return value == StampType.indian ? {
        indian: null
    } : value == StampType.pakistani ? {
        pakistani: null
    } : value;
}
